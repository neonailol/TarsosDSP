...
public class MouseCursorLayer implements Layer, MouseMotionListener, MouseListener {
    ...
    public void draw(Graphics2D graphics) {
        if (drawCursor) {
            ...
            if (!onlyDrawVertical) {
                graphics.drawLine(
                    Math.round(cs.getMax(Axis.X)),
                    Math.round((float) unitPoint.getY()),
            ...
            graphics.drawLine(
                Math.round((float) unitPoint.getX()),
                (int) Math.floor(cs.getMin(Axis.Y)),
                Math.round((float) unitPoint.getX()),
                (int) Math.ceil(cs.getMax(Axis.Y))
            );
        }
    }
