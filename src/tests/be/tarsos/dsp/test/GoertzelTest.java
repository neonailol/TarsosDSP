...
public class GoertzelTest {
    ...
    public static float[] testAudioBufferSine(final double f0, int size) {
        ...
        for (int sample = 0; sample < buffer.length; sample++) {
            ...
            buffer[sample] = (float) (
                amplitudeF0 * Math.sin(2 * Math.PI * f0
                                       * time)
            );
        }
        return buffer;
    ...
    public void testDetection() throws LineUnavailableException, UnsupportedAudioFileException {
        ...
        final float[][] floatSinBuffers = {testAudioBufferSine(6000, 10240), testAudioBufferSine(2000, 10240), testAudioBufferSine(
            4000,
            10240
        )};
    ...
    public void testDTMF() throws LineUnavailableException, UnsupportedAudioFileException {
        ...
        final float[][] floatSinBuffers = {DTMF.generateDTMFTone('1'), DTMF.generateDTMFTone('2'), DTMF.generateDTMFTone('3'),
                                           DTMF.generateDTMFTone(
                                               '4'), DTMF.generateDTMFTone('5'), DTMF.generateDTMFTone('6'), DTMF.generateDTMFTone('7'),
                                           DTMF.generateDTMFTone(
            '8'),
                                           DTMF.generateDTMFTone(
                                               '9')};
        ...
        final TarsosDSPAudioFloatConverter converter = TarsosDSPAudioFloatConverter.getConverter(JVMAudioInputStream.toTarsosDSPFormat(
            format));
        final byte[] byteBuffer = new byte[floatBuffer.length * format.getFrameSize()];
        assertEquals("Specified 16 bits so framesize should be 2.", 2, format.getFrameSize());
        converter.toByteArray(floatBuffer, byteBuffer);
        ...
        dispatcher.addAudioProcessor(new Goertzel(44100, stepSize,
            ...
            public void handleDetectedFrequencies(
                ...
                if (frequencies.length == 2) {
                    ...
                    int colIndex = -1;
                    for (int i = 0; i < 4; i++) {
                        if (frequencies[0] == DTMF.DTMF_FREQUENCIES[i] || frequencies[1] == DTMF.DTMF_FREQUENCIES[i]) { rowIndex = i; }
                    }
                    for (int i = 4; i < DTMF.DTMF_FREQUENCIES.length; i++) {
                        if (frequencies[0] == DTMF.DTMF_FREQUENCIES[i] || frequencies[1] == DTMF.DTMF_FREQUENCIES[i]) { colIndex = i - 4; }
                    }
