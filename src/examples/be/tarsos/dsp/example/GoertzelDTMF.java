...
public class GoertzelDTMF extends JFrame implements ActionListener {
    ...
    private final AudioProcessor goertzelAudioProcessor = new Goertzel(
        44100,
        stepSize,
        DTMF.DTMF_FREQUENCIES,
        new FrequenciesDetectedHandler() {
            ...
            public void handleDetectedFrequencies(
                ...
                if (frequencies.length == 2) {
                    ...
                    int colIndex = -1;
                    for (int i = 0; i < 4; i++) {
                        if (frequencies[0] == DTMF.DTMF_FREQUENCIES[i] ||
                            frequencies[1] == DTMF.DTMF_FREQUENCIES[i]) { rowIndex = i; }
                    }
                    for (int i = 4; i < DTMF.DTMF_FREQUENCIES.length; i++) {
                        if (frequencies[0] == DTMF.DTMF_FREQUENCIES[i] ||
                            frequencies[1] == DTMF.DTMF_FREQUENCIES[i]) { colIndex = i - 4; }
                    }
