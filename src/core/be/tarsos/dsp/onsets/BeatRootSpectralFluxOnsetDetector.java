...
public class BeatRootSpectralFluxOnsetDetector implements AudioProcessor, OnsetDetector {
    ...
    public boolean process(AudioEvent audioEvent) {
        ...
        for (int i = 0; i < fftSize / 2; i++) {
            if (reBuffer[i] > prevFrame[i]) { flux += reBuffer[i] - prevFrame[i]; }
        ...
        if (ltAverage == 0) { ltAverage = frameRMS; } else { ltAverage = ltAverage * decay + frameRMS * (1.0 - decay); }
        if (frameRMS <= silenceThreshold) {
            ...
            for (int i = 0; i < freqMapSize; i++) {
                ...
                if (frames[frameCount][i] < 0) { frames[frameCount][i] = 0; }
            }
        }
