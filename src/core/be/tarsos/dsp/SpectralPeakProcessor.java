...
public class SpectralPeakProcessor implements AudioProcessor {
     ...
     */
    public static List<SpectralPeak> findPeaks(
        float[] magnitudes,
        float[] frequencyEstimates,
        List<Integer> localMaximaIndexes,
        int numberOfPeaks,
        ...
        if (localMaximaIndexes.size() == 0) { return spectralPeakList; }
    ...
    public static final float percentile(double[] arr, double p) {
        ...
        if (p < 0 || p > 1) { throw new IllegalArgumentException("Percentile out of range."); }
