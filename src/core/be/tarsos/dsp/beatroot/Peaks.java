...
public class Peaks {
        ...
        while (mid < end) {
            ...
            if (maxp == mid) {
                ...
                for (j = peakCount; j > 0; j--) {
                    if (data[maxp] <= data[peaks[j - 1]]) { break; } else if (j < peaks.length) { peaks[j] = peaks[j - 1]; }
                }
                if (j != peaks.length) { peaks[j] = maxp; }
                if (peakCount != peaks.length) { peakCount++; }
            }
            mid++;
        }
        return peakCount;
     ...
     */
    public static LinkedList<Integer> findPeaks(
        ...
        while (mid < end) {
            ...
            if (maxp == mid) {
                if (overThreshold(data, maxp, width, threshold, isRelative, av)) {
                    if (debug) { System.out.println(" peak"); }
    ...
    public static double expDecayWithHold(
        double av, double decayRate,
    ...
    public static boolean overThreshold(
        double[] data, int index, int width,
        ...
        if (isRelative) {
            ...
            if (iStop > data.length) { iStop = data.length; }
            double sum = 0;
        ...
        } else { return (data[index] > threshold); }
    } // overThreshold()
