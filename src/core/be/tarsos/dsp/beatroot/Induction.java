...
public class Induction {
    ...
    public static AgentList beatInduction(EventList events) {
        ...
        if (intervals == 0) { return new AgentList(); }
        for (b = 0; b < intervals; b++)
        ...
        for (b = 0; b < intervals; b++)    // score intervals
            for (i = b + 1; i < intervals; i++) {
                ...
                submult = ratio < 1;
                if (submult) { degree = (int) Math.round(1 / ratio); } else { degree = (int) Math.round(ratio); }
                if ((degree >= 2) && (degree <= 8)) {
                    if (submult) { err = Math.abs(clusterMean[b] * degree - clusterMean[i]); } else {
                        err = Math.abs(clusterMean[b] -
                                       clusterMean[i] *
                                       degree);
                    }
                    if (err < (submult ? clusterWidth : clusterWidth * degree)) {
                        if (degree >= 5) { degree = 1; } else { degree = 6 - degree; }
        ...
        if (debug) { System.out.println(" IBI"); }
        return a;
    } // beatInduction()
